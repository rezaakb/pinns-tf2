...
def read_data_fn(root_path):
    ...
    data = pinnstf.utils.load_data(root_path, "cylinder_nektar_wake.mat")
...
def output_fn(outputs: Dict[str, tf.Tensor],
    ...
    outputs["v"] = -pinnstf.utils.gradient(outputs["psi"], x)
    outputs["u"] = pinnstf.utils.gradient(outputs["psi"], y)
...
def pde_fn(outputs: Dict[str, tf.Tensor],
    ...
    u_x, u_y, u_t = pinnstf.utils.gradient(outputs["u"], [x, y, t])
    u_xx = pinnstf.utils.gradient(u_x, x)
    u_yy = pinnstf.utils.gradient(u_y, y)

    v_x, v_y, v_t = pinnstf.utils.gradient(outputs["v"], [x, y, t])
    v_xx = pinnstf.utils.gradient(v_x, x)
    v_yy = pinnstf.utils.gradient(v_y, y)

    p_x, p_y = pinnstf.utils.gradient(outputs["p"], [x, y])
